      ...
      {reports.map((report) => (
        <div key={report.id} className="agency-card rounded-lg p-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-md bg-[#EFF6FF] flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex-shrink-0">
            {report.reportUrl ? (
              <a href={report.reportUrl} className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-[#1A73FF] rounded-md border border-[#E5E7EB] hover:bg-[#F9FAFB] transition-all">
